	...
	switch ($_GET['m']){
		...
		case 'create':
			...
			if(check_val('_POST',$v_exists,$v_empty,null,null)){
				if(create($_POST['show'], $_POST['title'], $_POST['theme'], $_POST['section'], $_POST['overview'], $_POST['href'])){
		...
		case 'save':
			...
			$v_empty=array('title','theme','section');
			if(check_val('_POST',$v_exists,$v_empty,null,null)){
				if(save($_POST['show'], $_POST['title'], $_POST['theme'], $_POST['section'], $_POST['overview'], $_POST['href'])){
					header('Location: '.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=list');
					exit();
				}
			}
			...
			exit();
		break;	}
	if(isset($data1) && trim($data1)!=''){ print $data1; }else{ print '<div align="center">- список пуст -</div>'; }
