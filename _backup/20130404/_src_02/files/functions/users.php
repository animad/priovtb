			...
			$data='<div class="block1">'.
				      '<table class="table1" width="100%">'.
					      ...
					      '<tr><td class="l"></td><td class="data">'.
						      ...
						      '<table class="table2" cellpadding="2" cellspacing="0" border="0" width="100%">'.
							      '<tr>'.
								      '<td>Логин</td>'.
									  '<td width="150"><input type="text" style="width: 150px;" id="user_login" value="'.$login.'" name="user_login"></td>'.
								  '</tr>'.
							      '<tr>'.
								      '<td>Пароль</td>'.
									  '<td width="150"><input type="password" style="width: 150px;" id="user_passw" value="" name="user_passw"></td>'.
						      '</table>'.
							  '<br />'.
							  '<div align="center">'.
...
//					  '<div align="center"><i>первый этап регистрации</i></div>'.
					  ...
					  '<form id="f_user_register" action="tool-user_create.php" method="post" style="margin: 0; padding: 0;">'.
						  ...
						  '<table class="table5" cellpadding="5" cellspacing="1" border="0">'.
							  '<tr'.$this->colrow($j1++).'>'.
				...
				if(trim($_POST['login'])!='' && trim($_POST['inn'])!='' && trim($_POST['fio'])!='' && trim($_POST['org'])!='' && trim($_POST['email'])!=''){
					...
					if(mysql_errno()<1){
						if(mysql_num_rows($res)<1){
							...
							if(strlen(trim($_POST['inn']))==$this->_vars['inn_size']){
								//--- проверяем e-mail
		...
		public function find_sc($sc=null){
			if($sc!=null && trim($sc)!=''){

				$q='select * from `users` where `session_code` like "'.addslashes(trim($sc)).'" limit 1';
				$res=mysql_query($q);
