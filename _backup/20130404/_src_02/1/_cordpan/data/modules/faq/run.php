...
//--- ИНТЕРФЕЙС ---
		case 'start':
			...
			if(isset($_GET['theme_id']) && trim($_GET['theme_id'])!=''){
				...
				//--- составление сиска ВОПРОСОВ-ОТВЕТОВ
				$data2='';
				$j1=0;
				...
				if(mysql_errno()<1){
					if(mysql_num_rows($res)>0){
						while($row=mysql_fetch_assoc($res)){
							if($row['on']==1){
								$t_show='<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=visibility_question&id='.$row['id'].'&v=0" class="hrf2" title="вылючить">выкл</a>';
							}else{
								$t_show='<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=visibility_question&id='.$row['id'].'&v=1" class="hrf3" title="включить">вкл</a>';
							}
							$data2[]='<tr'.colrow($j1++,$row['on']).'>'.
										 ...
										 '<td style="padding: 0; vertical-align: top;">'.
										     '<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=qa_delete&id='.$row['id'].'" class="hrf4" title="удалить" onClick="return confirm(\'Удалить ВОПРОС-ОТВЕТ №'.$row['id'].'?\')?true:false;">XX</a>'.
										 '</td>'.
									 '</tr>';
						}
						$data2='<form action="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=qa_change" method="post">'.
						       '<p align="center"><a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=qa_add&theme_id='.trim($_GET['theme_id']).'" class="hrf2">добавить пустые ВОПРОС-ОТВЕТ</a></p>'.
							   ...
							   '</form>';
					}else{ $data2='<p align="center">- ВОПРОСЫ-ОТВЕТЫ для темы не заданы -</p>'; }
					...
					if(mysql_num_rows($res)>0){
						...
						if(true){
							$tool='<div style="position: relative; line-height: 20px; width: 350px; text-align: right;">'.
									 '<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=delete_theme&id='.$row['id'].'" class="hrf1" title="удалить" onClick="return confirm(\'Тема опроса &quot;'.output($row['theme']).'&quot; \r\n\r\nУдалить с вопросами и ответами?\')">x</a>'.
									 '<div style="position: absolute; top: 0; left: 0; padding: 0;">'.
										 ($row['on']==1?'<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=visibility_theme&v=0&id='.$row['id'].'" class="hrf1" title="вкл / ВЫКЛ">выкл</a>':
														'<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=visibility_theme&v=1&id='.$row['id'].'" class="hrf1" title="ВКЛ / выкл">вкл</a>').
									 '</div>'.
								 '</div>';
						}else{ $tool=''; }
						
						$data1[]='<div style="position: relative; padding: 10px 0; left: 0; width: 100%; background-color: #DBF5D6; text-align: center;">'.
									 '<p align="center"><a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'" class="hrf2">закрыть редктирование опроса</a></p>'.
			...
			if(mysql_errno()<1){
				if(mysql_num_rows($res)>0){
					while($row=mysql_fetch_assoc($res)){
						if($row['on']==1){
							$t_show='<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=visibility_theme&id='.$row['id'].'&v=0" class="hrf3" title="вылючить">выкл</a>';
						}else{
							$t_show='<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=visibility_theme&id='.$row['id'].'&v=1" class="hrf3" title="включить">вкл</a>';
						}
						$data2[]='<tr '.colrow($j1++,$row['on']).'>'.
					...
					if(isset($data2)){
						$data1[]='<p align="center"><b>темы опросов</b></p>';
						$data1[]='<p align="center">'.
						             '<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=theme_add" class="hrf2">новая тема</a>'.
								 '</p>';
...
//--- ОБРАБОТКА ---
		...
		break;
		case 'theme_add':
			//--- добавляем пустую тему для ВОПРОС-ОТВЕТ - БАЗА
		...
		break;
		case 'qa_add':
			//--- добавляем пустые ВОПРОС-ОТВЕТ - БАЗА
			if(isset($_GET['theme_id']) && trim($_GET['theme_id'])!=''){
