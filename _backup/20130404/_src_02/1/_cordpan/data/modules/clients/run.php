			...
			if(mysql_errno()<1 && mysql_num_rows($res)>0){
				...
				while(list($key,$row)=each($pglist->list)){
					...
					$data2[]='<tr'.colrow($j1++,$row['on']).'>'.
								 ...
								 '<td>'.htmlspecialchars(trim($row['fio'])).'</td>'.
								 '<td>'.htmlspecialchars(trim($row['org'])).'</td>'.
								 ...
								 '<td style="padding: 0; text-align: center;">'.
									 ...
									 '<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=access&id='.$row['id'].'&v=deny" title="ОТКЛОНИТЬ" class="hrf6">––</a>'.
								 '</td>'.
							 '</tr>';
				}
