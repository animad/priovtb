<?php
	...
	class module{
		...
		var $tools;			//--- КЛАССИФИКАТОР СТАНДАРТНЫХ инструментов
		
		function module(){
			//--- КЛАССИФИКАТОР СТАНДАРТНЫХ инструментов
			$this->tools['editorHTML']=null;
			$this->tools['filemanager']=null;
		}
					...
					if(file_exists($fln1) && filesize($fln1)>0){
						...
						if($uid==null || $uid!=null && $uid==$tmp1['info']['uId']){
							...
							if($ok){
								...
								//--- регистрируем КЛАССИФИКАТОР СТАНДАРТНОГО инструмента
								$fln2=$_dir['modplace'].$entry.'/tool_interface.ini';
								if(file_exists($fln2) && filesize($fln2)){
									$ti=parse_ini_file($fln2,true);
									if(isset($ti['tool'])){
										$this->tools[$ti['tool']]['uId']=$tmp1['info']['uId'];
										if(isset($ti['config'])){
											$fln3=$_dir['modplace'].$tmp1['run']['place'].'/functions/'.trim($ti['config']);
											if(file_exists($fln3) && filesize($fln3)){
													include($fln3);
//													$tl=new 
											}
										}
