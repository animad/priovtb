...
//	$_modvars['btn']['start'][]=array($_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=categories','Категории имущества');
	...
	$_modvars['btn']['bail_edit'][]=array((isset($_SESSION['back'])?$_SESSION['back']:$_SERVER['HTTP_REFERER']),'Назад');
	$_modvars['btn']['bail_create'][]=array((isset($_SESSION['back'])?$_SESSION['back']:$_SERVER['HTTP_REFERER']),'Назад');
	$_modvars['btn']['bail_photo_manager'][]=array((isset($_SESSION['back'])?$_SESSION['back']:$_SERVER['HTTP_REFERER']),'Назад');
...
//--- ИНТЕРФЕЙС ---
		...
		break;
		case 'bail_edit':
			$data1[]='<p align="center"><b>Редактировать информацию об имуществе</b></p>';
			...
			if(isset($_SESSION['back'])){ unset($_SESSION['back']); }
		break;
		case 'bail_photo_manager':
			$data1[]='<p align="center"><b>Управление фотоматериалами</b></p>';

			//--- отображаем список категорий
			include($_dir['mod_func'].'/bail_photo_manager.inc');
		...
		case 'files':
			...
			switch ($_GET['t']){
				...
				case 'pictures':
					$data1[]='<p align="center">'.
								 '<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m='.$_GET['m'].'&t=description" class="hrf2">Файлы описаний имущества</a>'.
							 '</p>';
					include($_dir['mod_func'].'/files_pictures.inc');
				break;
				case 'description':
					$data1[]='<p align="center">'.
								 '<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m='.$_GET['m'].'&t=pictures" class="hrf2">Файлы картинок</a>'.
							 '</p>';
					include($_dir['mod_func'].'/files_description.inc');
				break;
			}
...
//--- ОБРАБОТКА ---
		...
		case 'bail_delete':
			if (isset($_GET['pid'])){
				...
				if(mysql_errno()<1){
					if(mysql_affected_rows()>0){
						$fln=$_modvars['place']['pages'].'/'.$_GET['pid'].'.html';
		...
		case 'bail_update':
			...
			if (isset($_POST['show']) && ''!=trim($_POST['show']) && isset($_POST['title']) && ''!=trim($_POST['title']) && isset($_POST['parent']) && ''!=trim($_POST['parent'])){
				if (isset($_POST['pid']) && ''!=trim($_POST['pid'])){
					$q='update `rp_list` set
						`on`="'.input($_POST['show']).'",
						`type`="obj",
						`title`="'.input($_POST['title']).'",
						`filename`='.(isset($_POST['descr_f']) && ''!=trim($_POST['descr_f'])?'"'.input($_POST['descr_f']).'"':'null').',
					...
					$q='insert into `rp_list`
						(`on`,`date`,`type`,`title`,`filename`,`parent`)
						values
						("'.input($_POST['show']).'",now(),"obj","'.input($_POST['title']).'",'.(isset($_POST['descr_f']) && ''!=trim($_POST['descr_f'])?'"'.input($_POST['descr_f']).'"':'null').',"'.input($_POST['parent']).'")';
					$res=mysql_query($q);
					print mysql_error();
					if (mysql_errno()<1){
						if (mysql_affected_rows()>0){
			...
			if (!isset($err)){
				...
				//--- если получаем ТЕКСТ, то сохраняем его
				if (isset($_POST['bail_text'])){
					$fln=$_modvars['place']['pages'].'/'.$pid.'.html';
					$fp=fopen($fln,'w');
					$ans=fwrite($fp,trim($_POST['bail_text']));
					chmod($fln,0777);
					fclose($fp);
				}
			...
			if (isset($_POST['pid'])){
				...
				if (isset($err)){
					...
					Header('Location: '.$_SERVER['HTTP_REFERER'].'&pid='.$pid);
				}
			}
		break;
		case 'bail_photo_unlink':
			if (isset($_GET['pid'])){
				$q='delete from `'.$_modvars['mysql']['images'].'` where `id`="'.input($_GET['pid']).'" limit 1';
				$res=mysql_query($q);
				if(mysql_errno()<1){
					if(mysql_affected_rows()>0){
						$err='Фотографии отключены';
					}else{ $err=$_comvars['err'][4]; }
				}else{ $err=mysql_error(); }
			}else{ $err=$_comvars['err'][1]; }
		...
		break;
		case 'bail_photo_add':
			if (isset($_POST['thumb']) && ''!=trim($_POST['thumb']) && isset($_POST['preview']) && ''!=trim($_POST['preview']) && isset($_POST['parent']) && ''!=trim($_POST['parent'])){
				$q='insert into `'.$_modvars['mysql']['images'].'`
					(`date`,`filename_thumb`,`filename_preview`,`rp_list_parent`)
					values
					(now(),"'.input($_POST['thumb']).'","'.input($_POST['preview']).'","'.input($_POST['parent']).'")';
				$res=mysql_query($q);
				if(mysql_errno()<1){
					if(mysql_affected_rows()>0){
						$err='Фотографии подключены';
					}else{ $err=$_comvars['err'][4]; }
				}else{ $err=mysql_error(); }
			}else{ $err=$_comvars['err'][1]; }
		...
		case 'visibility':
			...
			Header('Location: '.$_SERVER['HTTP_REFERER']);
		break;
		case 'visibility_pics':
			if(isset($_GET['v']) && trim($_GET['v'])!='' && isset($_GET['id']) && trim($_GET['id'])!=''){
				$q='update `'.$_modvars['mysql']['images'].'` set `on`="'.$_GET['v'].'" where `id`="'.$_GET['id'].'"';
				$res=mysql_query($q);
				if(mysql_errno()>0){ $err=mysql_error(); }
			}else{ $err=$_comvars['err'][3]; }
		...
		case 'visibility_file':
			if(isset($_GET['v']) && trim($_GET['v'])!='' && isset($_GET['fln']) && trim($_GET['fln'])!=''){
				if (file_exists(trim($_GET['fln'])) && filesize(trim($_GET['fln']))){
					...
					switch(trim($_GET['v'])){
						case '1':
							if ('-'==$tmp['basename'][strlen($tmp['basename'])-1]){
								$fln2=$tmp['dirname'].'/'.substr($tmp['basename'],0,strlen($tmp['basename'])-1);
		...
		case 'delete_file':
			//--- удаление ФАЙЛА
			if (file_exists(trim($_GET['fln'])) && filesize(trim($_GET['fln']))){
				if (is_writable(trim($_GET['fln']))){
					unlink(trim($_GET['fln']));
				}else{ $err='Нет доступа к файлу'; }
			}else{ $err='Запрошенный файл не найден'; }
				...
				if(trim($_FILES['file']['name'])!=''){
					if($_FILES['file']['error']<1){
						if($_FILES['file']['size']<'1500000'){
							$ud=substr($_modvars['place'][$_POST['place']],3);
							if(isset($ud) && trim($ud!='')){
								$ans=file_upload($ud);
