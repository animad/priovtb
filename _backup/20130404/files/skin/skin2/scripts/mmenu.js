...
//		alert(typeof(current_page));
			...
			if ((show==2 || show==1 && obj[level]["smenu"][i].show==1 || obj[level]["smenu"][i].show!=0) && obj[level]["smenu"][i].on==1){ //--- отображать ПУНКТ
				...
				if(obj[level]["smenu"][i].href=="" ){
					if(typeof(obj[level]["smenu"][i].file)=="undefined" || obj[level]["smenu"][i].file==""){ hrf1="#"; }else{ hrf1=obj[level]["smenu"][i].file; }
				}else{ hrf1="/?dr="+obj[level]["smenu"][i].href; }
				...
				if(sk==3){
					if(obj[level]["smenu"][i].href!="" || typeof(obj[level]["smenu"][i].file)!="undefined" && obj[level]["smenu"][i].file!=""){
						data1[level]+="<li"+bg+">"+
			...
			//---если пункт содержит подменю, то обрабатываем
