...
//--- ИНТЕРФЕЙС ---
		case 'start':
			...
			$j1=0;
			$data1[]='<p align="center"><b>создать новость</b></p>';
			$data1[]='<form action="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=create" method="post" style="padding: 0;">'.
			             '<table border="0" cellspacing="1" cellpadding="5" class="table1" align="center">'.
							 ...
							 '<tr'.colrow($j1++).'>'.
								 ...
								 '<td align="left"><textarea style="width: 500px" type="text" name="text">'.$text.'</textarea></td>'.
							 '</tr>'.
						 '</table>'.
			...
			if(mysql_errno()<1){
				if(mysql_num_rows($res)>0){
					...
					$data1[]='<p align="center"><b>размещенные новости</b></p>';
					...
					while(list($key,$row)=each($pglist->list)){
						...
						if(true){
							$tool='<div style="position: relative; line-height: 20px; width: 350px; text-align: right; margin: 0 auto;">'.
											 '<a href="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=delete&id='.$row['id'].'" class="hrf1" title="удалить" onClick="return confirm(\'Новость &quot;'.output($row['title']).'&quot; \r\n\r\nУдалить?\')">x</a>'.
			...
			if(isset($err)){
				...
				$data1[]='<form action="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=save" method="post" style="padding: 0;">'.
				             ...
				             '<table border="0" cellspacing="1" cellpadding="5" class="table1" align="center">'.
								 ...
								 '<tr'.colrow($j1++).'>'.
									 '<td align="left" valign="top" width="150">Полный текст</td>'.
									 '<td align="left"><textarea style="width: 500px" type="text" name="text">'.stripslashes(trim($text)).'</textarea></td>'.
								 '</tr>'.
							 '</table>'.
							 '<p align="center"><input type="submit" value="Ок"></p>'.
						 '</form>';
