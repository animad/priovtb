...
//--- ИНТЕРФЕЙС ---
		case 'start':
			...
			if(isset($_modvars['use_external_editorHTML']) && $_modvars['use_external_editorHTML'] && isset($mod->tools['editorHTML']['uId'])){
				...
				$editor[$k]['r']['work_element']='requirement';
				$editor[$k]['r']['css_body']='';
				$editor[$k]['r']['work_text']=trim($requirement);
				...
				$val_editor_init='';
				$val_editor='<textarea style="width: 500px" type="text" name="requirement">'.trim($requirement).'</textarea>';
			}
			...
			$data1[]='<form action="'.$_SERVER['PHP_SELF'].'?dr='.$_GET['dr'].'&m=create" method="post" style="padding: 0;">'.
			             '<table border="0" cellspacing="1" cellpadding="5" class="table1" align="center">'.
						     ...
						     '<tr class="even"><td valign="top">Требования</td><td>'.$val_editor.'</td></tr>'.
