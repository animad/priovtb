 ...
 */
	...
	function parse_mmenu_js($plist=null){
		...
		if(isset($data)){
			...
			$j1=0;
			while(list($key,$val)=each($plist)){
				if(isset($data[$key]['title_p']) && trim($data[$key]['title_p'])!=''){ $data[$key]['title']=trim($data[$key]['title_p'].(isset($data[$key]['title'])?' '.$data[$key]['title']:'')); }
				if(isset($data[$key]) && trim($data[$key]['title'])!='' && isset($data[$key]['raiting']) && trim($data[$key]['raiting'])=="1"){ //--- страницы без названи€ ”Ѕ»–ј≈ћ
