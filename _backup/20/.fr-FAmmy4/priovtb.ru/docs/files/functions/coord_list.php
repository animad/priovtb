...
//					          ($pval['jur_l']?'<p align="center"><table border="0" cellspacing="0" cellpadding="3" class="border_01"><tr><td><img src="pool/images/map/'.$pval['map'].'" border="0"></td></tr></table></p>':'<br />').
					...
					if ($pval['jur_l']){
						if (isset($pval['map_src']) && ''!=trim($pval['map_src'])){
							$v1=file('pool/src/coord_data/'.trim($pval['map_src']));
							print implode("\r\n",$v1);
						}else{
							print '<div style="padding: 10px 0;">'.
									  '<table border="0" cellspacing="0" cellpadding="3" class="border_01" align="center">'.
										  '<tr>'.
											  '<td>'.
												  '<img src="pool/images/map/'.$pval['map'].'" border="0" />'.
											  '</td>'.
										  '</tr>'.
									  '</table>'.
								  '</div>';
					...
					print '<table border="0" cellspacing="0" cellpadding="3" align="center"><tr>';
					...
					if($pval['jur_l']==1){
						...
						print '<td valign="top" width="50%" align="center"><br />';
						if (isset($pval['map_src']) && ''!=trim($pval['map_src'])){
							$v1=file('pool/src/coord_data/'.trim($pval['map_src']));
							print implode("\r\n",$v1);
						}else{
							print '<table border="0" cellspacing="0" cellpadding="3" class="border_01"><tr><td>'.
...
*/
	...
	//--- адреса и контактные данные по ДО
//	$ct - (fiz_ || jur_ ) - тип клиентов
	
	function coord_list2($t2=null,$ct='fiz_'){
		global $globals;
		...
		if(is_array($t2)){
			...
			while(list($key,$val)=each($t2)){
				$data2[]='<tr'.colrow($j1++).'>'.
						   ...
						   '</td>'.
				           '<td>'.
							   output((isset($globals['towns'][$val][$ct.'phone'])?'('.$globals['regions'][$globals['towns'][$val]['town']]['phone_code'].') '.$globals['towns'][$val][$ct.'phone']:'')).
						   '</td>'.
				         '</tr>';
			}
			if(isset($data2)){
